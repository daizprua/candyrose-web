...
function BookingPageInner() {
  ...
  const handleSubmitBooking = async () => {
    ...
    if (adultos > selectedRoom.capacityAdults || ninos > selectedRoom.capacityChildren) {
      setToast({ type: 'error', text: language === 'es' ? 'La ocupación excede la capacidad de esta habitación' : 'Occupancy exceeds this room capacity' });
      return;
    }
    if (edadesNinos.length !== ninos || edadesNinos.some((edad) => edad < 0 || edad > selectedRoom.childAgeMax)) {
      setToast({ type: 'error', text: language === 'es' ? `Registra una edad válida para cada niño (0 a ${selectedRoom.childAgeMax})` : `Enter a valid age for every child (0 to ${selectedRoom.childAgeMax})` });
      return;
    }
    try {
      setSubmitting(true);
          ...
          <div className="bg-white rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl relative my-auto">
             ...
             <div className="bg-zinc-50 p-5 rounded-2xl mb-5 border border-zinc-100">
                ...
                {ocupacionCotizada.recargoNoche > 0 && (
                  <p className="text-[10px] font-bold text-zinc-500 mt-1">Incluye ${ocupacionCotizada.recargoNoche.toFixed(2)} por noche de huéspedes adicionales</p>
                )}
             </div>
             <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="block">
                    ...
                    <span className="text-[9px] text-zinc-400">Máx. {selectedRoom.capacityAdults}</span>
                  </label>
                  <label className="block">
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block mb-1">{language === 'es' ? 'Niños' : 'Children'}</span>
                    <input type="number" min={0} max={selectedRoom.capacityChildren} value={ninos}
                      ...
                      className="w-full px-3 py-2 border border-zinc-200 rounded-xl text-sm outline-none focus:border-primary" />
                    <span className="text-[9px] text-zinc-400">Máx. {selectedRoom.capacityChildren}</span>
                  </label>
                </div>
                {edadesNinos.length > 0 && (
                  <div className="rounded-xl border border-zinc-200 p-3">
                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-2">{language === 'es' ? 'Edad de cada niño' : 'Age of each child'}</p>
                    <div className="grid grid-cols-2 gap-2">
                      {edadesNinos.map((edad, index) => (
                        <label key={index} className="text-xs font-bold text-zinc-500">
                          {language === 'es' ? `Niño ${index + 1}` : `Child ${index + 1}`}
                    ...
                    <p className="text-[9px] text-zinc-400 mt-2">{language === 'es' ? `Hasta ${selectedRoom.childAgeMax} años; mayores se registran como adultos.` : `Up to age ${selectedRoom.childAgeMax}; older guests count as adults.`}</p>
                  </div>
                )}
                <label className="block">
